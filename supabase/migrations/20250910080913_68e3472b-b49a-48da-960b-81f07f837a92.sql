-- Add nix_owner role to the user_role enum
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'nix_owner';
...
-- Only Nix owners can view surveillance data
CREATE POLICY "Nix owners can view all surveillance data"
ON public.disease_surveillance
FOR SELECT
USING (get_user_role(auth.uid()) = 'nix_owner');

-- Only Nix owners can manage surveillance data
CREATE POLICY "Nix owners can manage surveillance data"
ON public.disease_surveillance
FOR ALL
USING (get_user_role(auth.uid()) = 'nix_owner')
WITH CHECK (get_user_role(auth.uid()) = 'nix_owner');
